...
window.addEventListener('load',loadPercentage,true);
	...
	$('.subjName').bind('mouseenter',function(e) {
		var src = e.target.id;
		$("<div id=\"floatDiv\"><p id=\"lblTotal\">Loading...</p><p id=\"lblPresent\">Loading...</p><p id=\"lblAbsent\">Loading...</p></div>").insertAfter('#day');		
		var offset = $(this).offset();
		var left = offset.left;
		var top = offset.top;
		$('#floatDiv').css('position','fixed');		
...
function fetchData(subjname){	
	//alert("name = "+subjname+"user = "+user);
		$('#lblTotal')[0].innerHTML = "Total Classes : Loading..";
		$('#lblPresent')[0].innerHTML = "Present: Loading..";
		$('#lblAbsent')[0].innerHTML = "Absent: Loading..";
